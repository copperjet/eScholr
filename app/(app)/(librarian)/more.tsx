...
import { View, StyleSheet, ScrollView, Alert, Linking, Pressable, StatusBar, Platform } from 'react-native';
...
export default function LibrarianMore() {
  ...
  const sections: { title: string; items: MenuItem[] }[] = [
    {
      title: 'Library',
      items: [
        { icon: 'library-outline', label: 'Catalog', sublabel: 'Browse and manage books', onPress: () => router.push('/(app)/(librarian)/catalog' as any) },
        { icon: 'folder-outline', label: 'Collections', sublabel: 'Manage genres and shelves', onPress: () => router.push('/(app)/(librarian)/collections' as any) },
        { icon: 'people-outline', label: 'Patrons', sublabel: 'Search staff and students', onPress: () => router.push('/(app)/(librarian)/patrons' as any) },
        { icon: 'download-outline', label: 'Import Books', sublabel: 'Import from CSV file', onPress: () => router.push('/(app)/(librarian)/book-import' as any) },
        { icon: 'barcode-outline', label: 'Scan Barcode', sublabel: 'Scan to find or check out', onPress: () => router.push('/(app)/(librarian)/scan' as any) },
        { icon: 'settings-outline', label: 'Library Settings', sublabel: 'Loan periods, limits', onPress: () => router.push('/(app)/(librarian)/settings' as any) },
      ],
    },
    {
      title: 'School',
      items: [
        { icon: 'megaphone-outline', label: 'Announcements', onPress: () => router.push('/(app)/announcements' as any) },
      ],
    },
    {
      title: 'Notifications',
    ...
    },
    {
      title: 'Resources',
      items: [
        { icon: 'globe-outline', label: 'igaprep.com', sublabel: 'Homework, past papers & revision', onPress: () => Linking.openURL('https://igaprep.com').catch(() => {}) },
      ],
    },
    {
      title: 'Account',
        ...
        <SafeAreaView edges={['top']} style={{ backgroundColor: colors.brand.primary }}>
          <View style={styles.hero}>
            <View style={styles.heroRow}>
              <Avatar name={user?.fullName ?? 'L'} size={70} />
              <View style={styles.heroText}>
                ...
                <View style={styles.rolePill}>
                  <ThemedText style={styles.rolePillText}>LIBRARIAN</ThemedText>
                </View>
              </View>
            </View>
            {school && (
