  ...
  const rawSections: { title: string; items: MenuItem[] }[] = [
    ...
    // ── Governance — Super Admin only ────────────────────────────────────
    {
      ...
      items: [
        ...
        ...(can(role, 'assessment_config')  ? [{ icon: 'sparkles-outline' as IoniconsName,         label: 'Character Framework',   sublabel: 'Value names, rating scale, on/off',     onPress: () => router.push('/(app)/(admin)/character-framework' as any), module: 'exams' as ModuleKey }] : []),
        ...(can(role, 'assessment_config')  ? [{ icon: 'ribbon-outline' as IoniconsName,           label: 'Grading Scales',        sublabel: 'A* / A / B / … with 0–100 coverage check', onPress: () => router.push('/(app)/(admin)/grading-scales' as any), module: 'exams' as ModuleKey }] : []),
        ...(can(role, 'assessment_config')  ? [{ icon: 'person-add-outline' as IoniconsName,       label: 'Student Overrides',     sublabel: 'Per-student weight/exempt for joiners',  onPress: () => router.push('/(app)/(admin)/student-overrides' as any), module: 'exams' as ModuleKey }] : []),
