...
const CSV_HEADERS = ['first_name', 'last_name', 'email', 'role'];
const TEMPLATE_ROWS = [
  'Jane,Wanjiku,jane@school.edu,hrt',
  'John,Omondi,john@school.edu,st',
  'Alice,Mwangi,alice@school.edu,admin',
];
...
function parseCSV(text: string): ParsedRow[] {
  ...
  const start = firstLower.includes('first_name') || firstLower.includes('email') ? 1 : 0;

  return lines.slice(start).map((line) => {
    const cols = line.split(',').map((c) => c.trim().replace(/^"|"$/g, ''));
    const [first_name = '', last_name = '', email = '', role = ''] = cols;
    ...
    return {
      ...
      role: VALID_ROLES.includes(role as UserRole) ? (role as UserRole) : '',
          ...
          <View style={{ gap: Spacing.base }}>
            <View style={[styles.infoBox, { backgroundColor: colors.surfaceSecondary, borderColor: colors.border }]}>
              ...
              <ThemedText variant="caption" color="muted" style={{ marginTop: 8 }}>
                Valid roles: {VALID_ROLES.join(', ')}. Passwords are auto-generated and shown on the Done screen so you can hand them to staff.
              </ThemedText>
            </View>
